...
# =====================================================================
# 7. CHRONOLOGICAL OUT-OF-SAMPLE CHECK (long-only, positive surprise)
# =====================================================================
# Splitting by TIME, not randomly - a real edge should hold up in the
# second half too. If it's strong in one half and vanishes in the other,
# that's a sign of regime-specific noise, not a durable signal.
long_trades = trades_df[trades_df['Direction'] == 'Long'].copy()

event_date_lookup = merged_df[['Ticker', 'Date_Window', 'Event_Date']].drop_duplicates()
long_trades = long_trades.merge(event_date_lookup, on=['Ticker', 'Date_Window'], how='left')
long_trades = long_trades.sort_values('Event_Date').reset_index(drop=True)

split_point = len(long_trades) // 2
split_date = long_trades.loc[split_point, 'Event_Date']

first_half = long_trades.iloc[:split_point]
second_half = long_trades.iloc[split_point:]

print("\n" + "=" * 50)
print(f"Chronological split at {split_date.date()} (long-only trades)")
for label, half in [('First half (earlier)', first_half), ('Second half (later)', second_half)]:
    vals = half['Net_Return'].dropna()
    if len(vals) > 1:
        t_stat, p_val = stats.ttest_1samp(vals, 0)
        print(f"{label}: n={len(vals)}, {half['Event_Date'].min().date()} to {half['Event_Date'].max().date()}, "
              f"win rate={(vals > 0).mean():.1%}, mean net return={vals.mean():.4%}, t={t_stat:.2f}, p={p_val:.4f}")
    else:
        print(f"{label}: not enough trades (n={len(vals)})")

print("\nUnique tickers per half (checking the split isn't just a ticker-mix artifact):")
print(f"First half tickers ({first_half['Ticker'].nunique()}): {sorted(first_half['Ticker'].unique())}")
print(f"Second half tickers ({second_half['Ticker'].nunique()}): {sorted(second_half['Ticker'].unique())}")

# =====================================================================
# 8. EXPORT A CLEAN, ONE-ROW-PER-EVENT DATASET FOR MODEL BUILDING
# =====================================================================
# merged_df has one row per TRADING DAY per event (needed for the CAR
# work above). For ML you want one row per EVENT: your feature(s) and
# the thing you're trying to predict. Starting minimal on purpose -
# Surprise_Pct as the only feature, profitability at +20d as the label.
ml_rows = []
for (ticker, window), grp in merged_df.groupby(['Ticker', 'Date_Window']):
    car_20 = grp.loc[grp['event_day'] == after_days, 'CAR']
    if car_20.empty:
        continue
    ml_rows.append({
        'Ticker': ticker,
        'Event_Date': grp['Event_Date'].iloc[0],
        'Surprise_Pct': grp['Surprise_Pct'].iloc[0],
        'CAR_20d': car_20.iloc[0],
        'Profitable_20d': int(car_20.iloc[0] > 0),  # simple binary label to start with
    })

ml_dataset = pd.DataFrame(ml_rows).sort_values('Event_Date').reset_index(drop=True)
ml_dataset.to_csv('ml_dataset.csv', index=False)
print(f"\nSaved {len(ml_dataset)} rows to ml_dataset.csv")
